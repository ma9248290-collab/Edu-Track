...
// ==========================================
// 🧠 خوارزمية الذكاء الاصطناعي للأسماء (Fuzzy Search)
// ==========================================
...
// ==========================================
// 🤖 المساعد الآلي (EduBot AI)
// ==========================================
function toggleBotChat() {
    const w = document.getElementById("botChatWindow");
    w.style.display = w.style.display === "flex" ? "none" : "flex";
}

function handleBotEnter(e) {
...
function sendBotMessage() {
    ...
    // تحليل لغوي واستخراج الإجابة (NLP Rules)
    setTimeout(() => {
        const response = generateBotResponse(text);
        body.innerHTML += `<div class="bot-msg">${response}</div>`;
        body.scrollTop = body.scrollHeight;
    }, 500); // محاكاة تفكير البوت
}

function generateBotResponse(text) {
    const q = normalizeArabicName(text); // استخدام نفس دالة التوحيد لفهم أفضل
    
    const rules = [
        { keys: ["طالب", "اضافه", "تسجيل", "جديد", "اضيف"], ans: "عشان تسجل طالب، روح من القائمة لـ <b>'إدارة الطلاب'</b> واضغط على زر <b>'+ تسجيل طالب'</b>. وممكن برضه تفتح مجموعة معينة وتضيفه من جواها." },
        { keys: ["حضور", "غياب", "حصه", "سجل حضور", "احضر"], ans: "لتسجيل الحضور، افتح <b>'سجل الحضور'</b> من القائمة الجانبية، اختار الحصة، وتقدر تعمل سكان للباركود أو تكتب اسم الطالب في مربع البحث السريع وهيتسجل فوراً." },
        { keys: ["فلوس", "خزنه", "ماليات", "اشتراك", "دفع", "مصاريف"], ans: "نظام الحسابات موجود في صفحة <b>'الخزنة والماليات'</b>. بتختار الحصة، وتحدد سعر الطالب ونسبة السنتر، والسيستم هيحسبلك الإيرادات ويطرح المصروفات ويطلعلك 'صافي الربح' للحصة دي!" },
        { keys: ["مساعد", "قفل", "حمايه", "سكرتاريه", "سكرتير", "باسوورد", "رقم سري"], ans: "تقدر تقفل الصلاحيات الحساسة زي (الحذف، الماليات، النسخ) بالضغط على زر <b>'🔒 قفل الإدارة'</b> اللي فوق. عشان ترجعها هتحتاج الرقم السري (الافتراضي: 1234)." },
        { keys: ["باركود", "كاميرا", "سكان"], ans: "تقدر تستخدم جهاز الباركود العادي في أي خانة بحث، أو تدوس على زرار <b>'📷 كاميرا'</b> عشان تفتح كاميرا اللاب توب أو الموبايل وتقرأ الكود مباشرة." },
        { keys: ["تقرير", "طباعه", "بي دي اف", "pdf", "ملف"], ans: "لطباعة تقرير طالب، ادخل على 'إدارة الطلاب'، دوس على 'الملف' جنب اسم الطالب، هتلاقي زرار فوق اسمه <b>'📄 طباعة تقرير'</b> بيطبعلك كل حضوره ودرجاته." },
        { keys: ["نسخه", "نسخ", "احتياطي", "باك اب", "داتا"], ans: "أهم حاجة الأمان! روح لصفحة <b>'النسخ الاحتياطي'</b> من القائمة الجانبية، ودوس 'تحميل نسخة' عشان تنزل ملف فيه كل بياناتك. وتقدر تسترجعه من نفس الصفحة لو غيرت الجهاز." },
        { keys: ["شرف", "اوائل", "نقاط", "تميز", "سلوك"], ans: "السيستم بيحسب النقاط تلقائي (حضور+امتحانات+واجبات). وكمان تقدر تضيف 'نقاط تميز سلوكي' من جوه بروفايل الطالب. أوائل المجموعات بيظهروا في صفحة <b>'لوحة الشرف 🏆'</b>." },
        { keys: ["انذار", "واتس", "واتساب", "رساله", "ملاحظه", "خطر"], ans: "روح لصفحة <b>'تحت الملاحظة'</b>، السيستم هيجيبلك كل اللي غابوا مرتين أو رسبوا مرتين، وهتلاقي زرار يفتح الواتساب ويبعت رسالة إنذار مجهزة لولي الأمر فوراً." },
        { keys: ["مين عملك", "مين برمجك", "شيفو"], ans: "أنا EduBot 🤖، نظام إدارة حصص ذكي تم برمجته وتطويره بواسطة الباشمهندس العبقري <b>شيفو</b>! 💪" }
    ];

    for(let rule of rules) {
        if(rule.keys.some(k => q.includes(k))) return rule.ans;
    }

    return "عذراً يا هندسة، مفيش إجابة مباشرة لسؤالك في قاع بياناتي. 😅<br>حاول تسألني بكلمات زي: <b>إضافة طالب، حضور، فلوس، باركود، واتساب، تقارير، نسخ احتياطي</b>.";
}
...
function stopCameraScanner() {
    if(html5QrcodeScanner) { html5QrcodeScanner.clear().then(() => { document.getElementById('scannerModal').style.display = 'none'; }).catch(e => { document.getElementById('scannerModal').style.display = 'none'; }); } 
...
function onScanSuccess(decodedText) {
    ...
    if(targetInput) { targetInput.value = decodedText; const enterEvent = new KeyboardEvent('keypress', { key: 'Enter', code: 'Enter', keyCode: 13, which: 13, bubbles: true }); targetInput.dispatchEvent(enterEvent); }
}
...
function renderDashboardCharts() {
    ...
    const groupLabels = groups; const groupData = groups.map(g => students.filter(s => s.group === g).length); const colors = ['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444'];
...
function renderAtRiskStudents() {
    ...
    students.forEach(student => {
        ...
        if(reasons.length > 0) {
            atRiskCount++; const msg = encodeURIComponent(`تحذير من إدارة الدرس: الطالب ${student.name} مستواه متراجع. السبب: ${reasons.join(" و ")}. برجاء المتابعة!`); const waUrl = `https://wa.me/20${student.parentPhone.replace(/^0+/, '')}?text=${msg}`;
...
function renderFinanceTable() {
    ...
    const groupStudents = students.filter(s => s.group === session.group); if(groupStudents.length === 0) return tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;">لا يوجد طلاب في هذه المجموعة</td></tr>`;
...
function renderExpensesList() {
    ...
    const totalExpensesWithCenter = centerCut + totalAdditionalExp; document.getElementById("total-expenses").innerText = `${totalExpensesWithCenter} ج.م`; document.getElementById("total-expenses").title = `السنتر: ${centerCut} ج.م + مصروفات: ${totalAdditionalExp} ج.م`;
...
document.getElementById("addStudentForm").addEventListener("submit", function(e) { 
    ...
    if(students.some(s => s.code === code)) return showToast("هذا الكود مستخدم بالفعل لطالب آخر!", "error");
...
renderGroupCards(); renderTable(); populateDropdowns();
if(sessionStorage.getItem("isLoggedIn") === "true") setTimeout(renderDashboardCharts, 500);
